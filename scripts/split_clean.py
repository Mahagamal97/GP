...
def checkdir(cleandir):
    if os.path.exists(cleandir) is False:
        os.makrdirs(cleandir)

root_path = '/media/hamza/4A128378128367B1/GP/cloneGP'
...
for file, fileNames in zip(clean_path, podcastNames):
    interval = 44100 * 10       # set intervals equal to 10 seconds
    #print("file = ",file,"...fileNames = ",fileNames)   
    
    wav_path = os.path.join(root_path, 'wav_dir', fileNames)
